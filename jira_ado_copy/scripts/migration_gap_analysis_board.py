"""migration_gap_analysis_board.py — Strict source-of-truth gap analysis: entire Jira board → ADO board.

Copy of migration_gap_analysis.py, adapted to accept an ENTIRE Jira board (by project key)
instead of a saved Jira filter ID. Kept as a separate file so the existing filter-based
flow (migration_gap_analysis.py) is never touched or put at risk.

Classification:
  ❌ MISSED      — Key present on the Jira board, NOT found anywhere in ADO
...
Usage:
  python3 migration_gap_analysis_board.py \\
    --jira-board-key OP \\
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 1 — JIRA: paginated board (project key) fetch (source of truth)
# ══════════════════════════════════════════════════════════════════════════════

def _paginate_jql(jira_client, jql: str, label: str) -> list[str]:
    """Cursor-based pagination over /rest/api/3/search/jql for a given JQL string."""
    keys = []
    page = 1
    ...
    logger.info(f"Paginating {label} (cursor-based) …")
    while True:
        resp = None
...
def fetch_all_jira_keys_by_board(jira_client, project_key: str) -> list[str]:
    """
    Fetch ALL issues for an entire Jira board (project) — no filter ID needed.
    Builds JQL directly from the project key: 'project = "KEY" ORDER BY created DESC'.

    Returns sorted list of Jira issue keys.
    """
    jql = f'project = "{project_key}" ORDER BY created DESC'
    logger.info(f"Board {project_key} JQL: {jql}")
    keys = _paginate_jql(jira_client, jql, f"Jira board {project_key}")
    logger.info(f"✅ Jira source of truth: {len(keys)} cards from board {project_key}")
    return keys
...
def print_report(
    ...
    print(f"  Jira Board  : {jira_board_key}  (entire board)")
    ...
    _section("📊  OVERVIEW")
    print(f"  {'Jira board cards (source of truth):':<45} {total_jira:>6}")
...
def print_remediation(classified: dict, board_name: str, ado_project: str):
    ...
    if missed:
        ...
        print(f"  python3 jira_ado_copy/scripts/worker_jira_to_ado_copy.py \\")
        print(f"    --jira-keys \"{keys_arg}\" \\")
        print(f"    --ado-team-name \"{board_name}\" \\")
...
def main():
    parser = argparse.ArgumentParser(
        description="Strict migration gap analysis: entire Jira board vs ADO board"
    )
    parser.add_argument("--jira-board-key", required=True,
                        help='Jira project key for the entire board (e.g. "OP")')
    ...
    parser.add_argument("--prefix",
                        help="Jira key prefix to use for ADO-wide scan (defaults to --jira-board-key)")
    ...
    # ── Step 1: Jira source of truth — entire board (project key), paginated ──
    jira_keys = fetch_all_jira_keys_by_board(jira_client, args.jira_board_key)
    if not jira_keys:
        logger.error("No Jira cards returned — check the board/project key and credentials")
        sys.exit(1)

    prefix = args.prefix or args.jira_board_key.upper()
    ...
    print_report(
        ...
        args.jira_board_key, board_name, ado_project, expected_area,
    )
